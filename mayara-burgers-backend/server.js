/*
============================================================
| JAVASCRIPT FINAL E DINÂMICO - MAYARA BURGUER'S           |
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // ALTERAÇÃO: Variáveis globais para guardar os dados vindos do servidor
    let todosOsProdutos = [];
    let listaDeAdicionais = []; // A lista agora virá da API

    // 2. DELEGAÇÃO DE EVENTOS (sem alterações)
    ...
    async function carregarDadosIniciais() {
        try {
            // ALTERAÇÃO: Busca produtos e adicionais ao mesmo tempo
            ...
            listaDeAdicionais = await responseAdicionais.json(); // Guarda a lista de adicionais globalmente
    ...
    function renderizarMenu() {
        const categorias = todosOsProdutos.reduce((acc, produto) => {
            // CORREÇÃO: Acessa o nome da categoria de forma segura
        ...
        for (const nomeCategoria in categorias) {
            ...
            // Lógica para subcategorias (sem alterações)
    ...
    function preencherEabrirModal(produto) {
        ...
        // A lógica de Pães permanece, pois depende de colunas do produto
        let htmlPao = (nomeCategoria === 'Lanches' || nomeCategoria === 'Hambúrgueres') // Adaptei para incluir Hambúrgueres
        ...
        // ALTERAÇÃO: Usa a listaDeAdicionais vinda da API
        let htmlAdicionais = '';
        // Mostra adicionais para hambúrgueres (ou outras categorias que você queira)
        if (nomeCategoria === 'Hambúrgueres' && listaDeAdicionais.length > 0) {
    ...
    // Funções de contar adicionais, atualizar preço, e eventos do modal (sem alterações)
    ...
    // 5. GESTÃO DO CARRINHO (sem alterações)
    ...
    // 6. GESTÃO DE CLIENTE E FINALIZAÇÃO (sem alterações)
