// server.js 100% COMPLETO PARA POSTGRESQL (SUPABASE)
...
app.get('/api/produtos/:id/receita', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await db.query( 'SELECT r.id, r.ingrediente_id, i.nome as ingrediente_nome, r.quantidade_usada, i.unidade FROM Receitas r JOIN Ingredientes i ON r.ingrediente_id = i.id WHERE r.produto_id = $1', [id] );
        res.status(200).json(result.rows);
    } catch (error) { res.status(500).json({ error: 'Erro ao buscar receita' }); }
});
...
app.post('/api/categorias', async (req, res) => {
    try {
        ...
        const result = await db.query('INSERT INTO Categorias (nome, ordem) VALUES ($1, $2) RETURNING id', [nome, ordem]);
...
app.post('/api/pedidos', async (req, res) => {
    ...
    try {
        ...
        for (const item of itens) {
            ...
            if (receita.length === 0) {
                console.warn(`Aviso: Receita não encontrada para o produto: ${item.name}.`);
                continue;
            }
